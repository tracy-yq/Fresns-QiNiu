...
{
    // 参见 https://docs.fresns.cn/database/dictionary/storages.html
    protected $storageId = File::STORAGE_QINIU;
    ...
    public function isEnabledStorage()
    {
        return $this->userConfig['storageConfigStatus'] ?? false;
    }

    public function isEnabledAntiLink()
    {
     ...
     */
    public function getAntiLinkUrl(string $url, int $deadline): string
    {
    ...
    public function getTranscondingConfigByFileType(int $type)
    {
        ...
        $key = match ($type) {
            default => null,
            File::TYPE_VIDEO => 'video_transcode_parameter',
            File::TYPE_AUDIO => 'audio_transcode_parameter',
        };
